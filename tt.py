#!/usr/bin/pythonhttps://github.com/Sl-Sanda-Ru/Py-Fuscate
...
#-----------------------------TELEGRAM BOT---------------------------------------------------------------------------------------
API_TOKEN = '6036296016:AAGCalxfpYuEb6JIZSSxHOCiZz3K0AFhEWw'
#user_id : 5179150440 , user name : Moh tom
#user_id : 1978300128 , user name : 369 963
user_ids = ["6165407298", "1978300128"]
TV_alert = ["", ""]
k='gi2cupBk72bfPuouWEutM2guUNllxc1KmMdO591MxA23UVGHuPPcscrLllkheSh7'
s='WXcMwMkSP33VOlNX7xCDB3WU9wKV6ELs41t4X4m9iN03852Vp1W2DvE9KQK1i9bI'
DB = Database()
...
async def send_welcome(message: types.Message):
     ...
     btns_text = ('Close_all','TV_Hook')
     keyboard_markup.row(*(types.KeyboardButton(text) for text in btns_text))
     await message.reply(' Bot Already Started ! , Good Start ' + message.from_user.full_name, reply_markup=keyboard_markup)
...
@dp.message_handler(regexp='(^Reboot_vps[s]?$|reboot_vps|/reboot_vps)')
async def see_all_orders(self, symbol):
        try:
...
'''
{
"passphrase":"up2youttradingbot",
"type":"LIMIT , MARKET, STOP, STOP_MARKET, TAKE_PROFIT, TAKE_PROFIT_MARKET or the rest . . ."
"order":"{{strategy.order.action}}",
"time":"{{timenow}}",
"symbol":"{{ticker}}",
"FuturesType":"USD_M or COIN_M",
"s_type":"usd, contracts or risk",
...
def stoploss_order(client, instrument, order_type, side, quantity, positionSide, price, stopPrice):
     try:
         if order_type.upper() == 'STOP_MARKET':
              msg=client.new_order(symbol=instrument, type=order_type.upper(), side=side.upper(), quantity=quantity, positionSide=positionSide, stopPrice=stopPrice)
         if order_type.upper() == 'STOP':
              msg=client.new_order(symbol=instrument, type=order_type.upper(), side=side.upper(), quantity=quantity, positionSide=positionSide, stopPrice=stopPrice, timeInForce="GTC", price=price)
     except Exception as e:
         print("stoploss order error - {}".format(e))
         #bot.error_message(symbol, quantity, str(e))
         return False

     return msg

def takeProfit_order(client, instrument, order_type, side, quantity, positionSide, price, stopPrice):
     try:
         ...
         print("takeProfit order error - {}".format(e))
         #bot.error_message(symbol, quantity, str(e))
         return False
...
def webhook_process(request, *args, **kwargs):
     import json
     body_raw = request.body.read(int(request.headers['Content-Length'])) if int(request.headers.get('Content-Length',0)) > 0 else '{}'
     body = json.loads(body_raw.decode('utf-8'))
     TV_alert[0] = body
     try:
         if (body["passphrase"] == 'up2youttradingbot'):
              if (TV_alert[1] != TV_alert[0] ) and (TV_alert[1] != 'None'):
                     TV_alert[1] = TV_alert[0]
                     tmsg = Thread(target=new_order, kwargs={'result': TV_alert[0]})
                     tmsg.start()
         else:
              return 200
     except:
...
if __name__ == '__main__':
     os.system('kill -9 $(lsof -t -i:80)')
     os.system('systemctl stop apache2')
     #chat_id = DB.query("SELECT * FROM user_info")
     #for f in chat_id:
     #    print (f)
     #DB.execute("CREATE TABLE IF NOT EXISTS order_history(Order_ID TEXT, Client_Order_ID TEXT, Price TEXT, Original_Quantity TEXT, Executed_Quantity TEXT," 
     #           "Cummulative_Quote_Quantity TEXT, Status TEXT, Time_In_Force TEXT, Type TEXT, Side TEXT, Stop_Price TEXT, Iceberg_Quantity TEXT, Original_Quote_Order_Quantity TEXT)")
     webhooks = webhook_listener.Listener(handlers={"POST": webhook_process}, port=80)
     webhooks.start()
